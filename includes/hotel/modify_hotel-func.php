...
if(isset($_POST['hotel'])){
  ...
  for ($i=1;$i<4;$i++){
    if ($_FILES["hotelimg$i"]["name"] != NULL) {
      $target_file = $target_dir . basename($_FILES["hotelimg$i"]["name"]);
      $uploadOk = 1;
      $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

      // move_uploaded_file($_FILES["hotelimg"]["tmp_name"][$i], $target_file);
      $check = getimagesize($_FILES["hotelimg$i"]["tmp_name"]);
      if($check !== false) {
      ...
      // Check file size
      if ($_FILES["hotelimg$i"]["size"] > 5000000) {
      ...
      if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["hotelimg$i"]["tmp_name"], $target_file)) {
          echo "The file ". basename( $_FILES["hotelimg$i"]["name"]). " has been uploaded.";
        } else {
  ...
  if($updateResult==true){
    ...
    header("Location: ../../hotelupdate.php?info=false");
    exit();
  }
}
